...
@Namespace("/sys")
@Component
public class ActionInsuranceType extends BaseAction implements ModelDriven<InsuranceType> {
	@Autowired
	private BizService service;
	private int uid;
	private InsuranceType bean = new InsuranceType();

	@Action(value = "add2InsuranceType", results = { @Result(name = "add2", location = "/WEB-INF/jsp/safe/addInsuranceType.jsp") })
	public String add2() {
		return "add2";
	}

	@Action(value = "getInsuranceType", results = { @Result(name = "getOne", location = "/WEB-INF/jsp/safe/modifyInsuranceType.jsp") })
	public String get() {
		try {
			InsuranceType temp = (InsuranceType) service.get(InsuranceType.class, uid);
	...
	@Action(value = "deleteInsuranceType")
	public String delete() {
		try {
			service.delete(InsuranceType.class, ids);
	...
	@Action(value = "updateInsuranceType")
	...
	@Action(value = "queryInsuranceType", results = { @Result(name = "queryList", location = "/WEB-INF/jsp/safe/listInsuranceType.jsp") })
	public String query() {
		try {
			int pageNum = 0;
				...
				for (Object o : parm) {
					...
					if (name.startsWith("s_")) {
						...
						if (StringUtil.notEmpty(fieldValue)) {
							...
							parmnames.add(name);
							parmvalues.add(FieldUtil.format(InsuranceType.class, name, fieldValue));
						}
					}
				}
			...
			Page page = null;
			page = service.find(p, InsuranceType.class);
