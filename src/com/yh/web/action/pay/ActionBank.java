...
@Namespace("/sys")
@Component
public class ActionBank extends BaseAction implements ModelDriven<Bank> {
	@Autowired
	private BizService service;

	private int uid;
	private Bank bean = new Bank();

	@Action(value = "add2Bank", results = { @Result(name = "add2", location = "/WEB-INF/jsp/pay/addBank.jsp") })
	public String add2() {
		return "add2";
	}

	@Action(value = "getBank", results = { @Result(name = "getOne", location = "/WEB-INF/jsp/pay/modifyBank.jsp") })
	public String get() {
		try {
			Bank temp = (Bank) service.get(Bank.class, uid);
	...
	@Action(value = "deleteBank")
	public String delete() {
		try {
			service.delete(Bank.class, ids);
	...
	@Action(value = "queryBank", results = { @Result(name = "queryList", location = "/WEB-INF/jsp/pay/listBank.jsp") })
	public String query() {
		try {
			int pageNum = 0;
				...
				for (Object o : parm) {
					...
					if (name.startsWith("s_")) {
						...
						if (StringUtil.notEmpty(fieldValue)) {
							...
							parmnames.add(name);
							parmvalues.add(FieldUtil.format(Bank.class, name, fieldValue));
						}
					}
				}
			...
			Page page = null;
			page = service.find(p, Bank.class);
